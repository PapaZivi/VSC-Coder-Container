...
Für den ATC-Hotfix ist der Start über `Setup-NewCodexComputer.cmd` empfohlen.
Der CMD-Launcher verwendet `Run-Setup.ps1`, sodass Syntax-/Integritätsprüfung und
das eigentliche Setup nicht mehr in zwei getrennten PowerShell-Prozessen laufen.
...
- Reiner Bitdefender-/ATC-Hotfix auf Basis des stabilen v1-Zweigs. Keine Claude-Code-/v2-Funktionen enthalten.
- Der CMD-Launcher startet nicht mehr erst einen separaten Syntaxcheck-Prozess und danach das Setup. `Run-Setup.ps1` prüft Syntax und Chatimport-Helper und führt das eigentliche Setup anschließend im selben PowerShell-Prozess aus.
- Docker Desktop wird nicht mehr automatisch aus PowerShell gestartet. Wenn die Engine noch nicht läuft, fordert das Setup zu einem manuellen Start auf. Dadurch hängt der langlebige Docker-Desktop-Prozess nicht mehr am PowerShell-Prozessbaum.
- VS Code wird für Bootstrap und Abschluss nicht mehr automatisch als GUI-Kindprozess des Setups gestartet. Das Setup fordert stattdessen zum manuellen Öffnen auf.
- Installierte VS-Code-Erweiterungen werden direkt aus den Extension-Verzeichnissen gelesen. Der bisherige zusätzliche `code --list-extensions --show-versions`-Prozess entfällt.
- Fehlende Erweiterungen werden in einem einzigen gebündelten `code --install-extension`-Aufruf installiert. Bereits vorhandene Erweiterungen verursachen überhaupt keinen VS-Code-CLI-Aufruf.
- Zusätzliche ATC-Checkpoints markieren den manuellen Docker-/VS-Code-Bootstrap und die gebündelte Extension-Installation.
- Der bereits in v1 vorhandene isolierte Chatimport bleibt unverändert: keine zweite Windows-PowerShell, kein Docker-Socket, kein Netzwerk und inkrementeller Import.
...
Ursache war die unterschiedlich gestartete
